...
var articleService= (function(){
    ...
    if (validateArticle(article)) {
        ...
        if (length + 1 === articles.push(article)) {
            return true;
        }
    }
    return false;
            ...
            for(var key in filterConfig) {
                switch (key) {
                    ...
                    case 'summary':
                        ...
                        break;
                    case 'createdAt':
                        if (elem.createdAt!==filterConfig.createdAt) {
                            return false;
                        }
                        break;
                } 
...
var articleRenderer = (function () {
    ...
    function renderArticle(article) {
        ...
        template.content.querySelector('.article-title').innerHTML = '<a href="news/'+article.id+'html">'+article.title+'</a>';
        ...
        template.content.querySelector('.imagecontainer').innerHTML='<img src="img/img/'+article.id+'.jpg">';
        template.content.querySelector('.news-dateposted-date').textContent = formatDate(article.createdAt);
        
        
        template.content.querySelector('.imagecontainer-link').setAttribute("href",newslink);
        template.content.querySelector('.more-link').setAttribute("href",newslink);
...
function showSearchPannel(){
    if(searchshown){
        document.querySelector('.search-pannel').style.display='none';
        searchshown=false;
    }else{
...
function listOfAuthors(){
    ...
    for(var i=0;i<auths.length;i++){
        list=list+makeAuthorListString(auths[i]);
    }
    return list;
}
